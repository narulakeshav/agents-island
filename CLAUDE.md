...
- If macOS asks for file/automation permissions after every rebuild, ad-hoc signing is the cause. Create/use a persistent local Code Signing identity named `Agents Island Local`, or set `AGENTS_ISLAND_CODESIGN_IDENTITY` before install.
...
## Inspect state
- `~/.agents-island/sessions/<tabUUID>.json` — exact payload the daemon reads.
